...
public:
                    ...
                    ~FVGTradeFilter()
                       {
                        if(m_detector!=NULL){ delete m_detector; m_detector=NULL; }
                       }
   ...
   void              Init(const string symbol, const ENUM_TIMEFRAMES tf, const bool enabled)
                       {
                        ...
                        if(!m_detector.Init(m_symbol, m_timeframe, cfg))
                          {
                           delete m_detector; m_detector=NULL;
                           m_enabled=false;
                           return;
                          }
                        // initial pass
   ...
   void              SetRadius(double radiusPts){ m_radiusPts = MathMax(50.0, radiusPts); }
                        ...
                        for(int i=0;i<ArraySize(fvgs);i++)
                          {
                           ...
                           if(fvgHigh<fvgLow){ double t=fvgHigh; fvgHigh=fvgLow; fvgLow=t; }
